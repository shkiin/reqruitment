 ...
 */
class superjob_elementor_blog extends Widget_Base {
	/**
	 * Define our get_name settings.
	 */
	public function get_name() {
		return 'superjob-blog';
	}
	/**
	 * Define our get_title settings.
	 */
	public function get_title() {
		return __( 'Blog Widget', 'superjob' );
	}
	/**
	...
	protected function register_controls()
    {
        ...
        $this->start_controls_section(
            'superjob_blog_section',
            [
                'label' => esc_html__('Blog Options', 'superjob'),
            ]
        );
        $this->add_control(
            'superjob_blog_get_category',
            [
                'label' => __('Select Category', 'superjob'),
                'type' => Controls_Manager::SELECT2,
                'default' => array(),
                'options' => superjob_blog_get_category(),
                'multiple' => true

            ]
        );
        $this->add_control(
            'superjob_blog_post_count',
            [
                'label' => __('No. Of Posts', 'superjob'),
	...
	protected function render() {
        ...
        $taxs = $settings['superjob_blog_get_category'];
        $posts_count = $settings['superjob_blog_post_count'];
         ...
         if ($query->have_posts()):
            ...
            <div class="blog-element">
                        ...
                        while ($query->have_posts()) : $query->the_post();
                            ...
                            $image = wp_get_attachment_image_src($post_thumbnail_id, 'superjob-blog-thumbnail-img');
                            ...
                            if($loop>=1) :
                                ...
                                <article class="blog-post">
                                    ...
                                    <h3 class="entry-title"><a href="<?php echo esc_url(get_the_permalink()); ?>"><?php the_title(); ?></a></h3>
                                            ...
                                            ?>
                                            <p class="post-excerpt"><?php echo wp_kses_post(superjob_get_excerpt(get_the_ID(), 125)); ?></p>
                                        </div>
                                    </article>
...
Plugin::instance()->widgets_manager->register_widget_type( new superjob_elementor_blog() );
